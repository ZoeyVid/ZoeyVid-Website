function loadView() {
    ...
    .then((json) => {
        ...
        json.links.forEach(element => {
          document.getElementById("links").innerHTML += '<a href="' + element.url + '"><div class="text-center text-white border-white border-2 text-2xl font-bold py-6 px-12 rounded w-min h-min place-items-center hover:shadow-lg">' + element.name + '</div></a>';
        });
    })
}
